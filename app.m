...
function app()
    ...
    % 
    % % Create a button for "Mackenzies_ML"
    % btnObstacleCrossing = uibutton(fig, 'push', 'Text', 'MP_ML Process', ...
    %     'Position', [50, 300, 200, 40], ...
    %     'ButtonPushedFcn', @(btn,event) MLCallback(fig));
...
% Callback function for Obstacle crossing data button
function MLCallback(fig)
    % List of subject folders (you could replace this with dynamic folder listing)
    
    dataPath = fullfile(pwd, 'MP_ML', 'Data');
    ...
    % Process Data
    subjects = process(selectedFolders, frameRate, method);
    disp('Processing finished')
end
